    ...
    <div className="p-6">
      ...
      <div className="bg-white rounded-lg shadow-sm overflow-hidden">
        ...
        <div className="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200">
           {/* Ton code de pagination actuelimport { useEffect, useState } from 'react';
import { NavLink } from 'react-router-dom';
import AjtEntreprise from './AjtEntreprise';
import { getAllEntreprise, Entreprise } from './entrepriseService';
import { ClipLoader } from 'react-spinners';
...
function typeBadge(type: string | null | undefined) {
  if (!type) return <span className="px-2 py-0.5 text-[10px] font-bold rounded-full bg-gray-100 text-gray-400">—</span>;
...
function Entreprises() {
  ...
  const filteredEntreprises = entreprisesData.filter((e) => {
    ...
    const matchType =
      filterType === '' ? true :
      filterType === '__none__' ? (!e.type_entreprise || e.type_entreprise.trim() === '') :
      e.type_entreprise === filterType;
  ...
  // Count per type for badge counters
  const countByType = (value: string) => {
    if (value === '') return entreprisesData.length;
    if (value === '__none__') return entreprisesData.filter(e => !e.type_entreprise || e.type_entreprise.trim() === '').length;
        ...
        {TYPE_OPTIONS.map((opt) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold border transition-all shadow-sm
                ${isActive
            ...
            >
              {opt.label}
              <span className={`text-[10px] font-black px-1.5 py-0.5 rounded-full ${isActive ? 'bg-white/60' : 'bg-gray-100'}`}>
                {count}
              </span>
            </button>
          );
...
export default Entreprises; fonctionne très bien ici */}
           <p className="text-sm text-gray-700">
